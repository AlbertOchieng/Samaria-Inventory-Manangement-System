	...
	if (divRequest == 'add') {
		...
		$('#createOrderForm').unbind('submit').bind('submit', function() {
			...
			for (var x = 0; x < categoryName.length; x++) {
				var categoryNameId = categoryName[x].id;
				if (categoryName[x].value == '') {
					$('#' + categoryNameId + '').after(
						'<p class="text-danger"> Category Name Field is required!! </p>'
					);
					$('#' + categoryNameId + '').closest('.form-group').addClass('has-error');
				} else {
					$('#' + categoryNameId + '').closest('.form-group').addClass('has-success');
				}
			} // for
			...
			for (var x = 0; x < quantity.length; x++) {
				...
				if (quantity[x].value == '') {
					$('#' + quantityId + '').after('<p class="text-danger"> Quantity Name Field is required!! </p>');
			...
			if (paid && discount && paymentType && paymentStatus) {
				if (validateProduct == true && validateQuantity == true && validateCategory == true) {
					// create order button
	...
	$.ajax({
		...
		success: function(response) {
			...
			tr +=
				...
				'<td style="padding-left:25px;">' +
				'<div class="form-group">' +
				'<input type="number" onkeydown="javascript: return event.keyCode === 8 || event.keyCode === 46 ? true : !isNaN(Number(event.key))"  name="quantity[]" id="quantity' +
				count +
				'" onchange="getTotal(' +
				count +
				')" onkeyup="getTotal(' +
...
function paymentOrder(orderId = null) {
	if (orderId) {
		...
		$.ajax({
			...
			dataType: 'json',
			success: function(response) {
				//  alert(JSON.stringify(response.order));

				// due
				$('#due').val(response.order[8]);

				// pay amount
				$('#payAmount').val(response.order[8]);

				var paidAmount = response.order[7];
				var dueAmount = response.order[8];
				var grandTotal = response.order[6];
				var updateDate = response.order[10];
				...
				$('#updatePaymentOrderBtn').unbind('click').bind('click', function() {
					...
					if (paymentType == '') {
						$('#paymentType').after('<p class="text-danger">The Pay Type field is required</p>');
						$('#paymentType').closest('.form-group').addClass('has-error');
					} else {
						$('#paymentType').closest('.form-group').addClass('has-success');
					}
					...
					if (paymentStatus == '') {
						$('#paymentStatus').after('<p class="text-danger">The Pay Status field is required</p>');
						$('#paymentStatus').closest('.form-group').addClass('has-error');
					} else {
						$('#paymentStatus').closest('.form-group').addClass('has-success');
					}
...
function getSubCategoryData(row = null) {
	if (row) {
		var productId = $('#categoryName' + row).val();

		if (productId == '') {
			$('#productName' + row).val('');

			// document.writeln('I am fucked up')
			...
			//	var productValue = $("#categoryName"+row).val()xcsdf

			//	if($("#categoryName"+count).val() == "") {
			//	$("#categoryName"+count).find("#changeProduct"+productId).removeClass('div-hide');
			//	console.log("#changeProduct"+count);
			//}
			// } // /for
		} else {
			$.ajax({
				url: 'php_action/ProductArray.php',
				type: 'post',
				data: { productId: productId },
				dataType: 'json',
				success: function(response) {
					...
					$('#productName' + row).empty();

					$('#productName' + row).prepend("<option value=''>" + '~~SELECT~~' + '</option>');
					for (var i = 0; i < len; i++) {
						var id = response[i]['id'];
						var name = response[i]['name'];

						//$("#productName"+row).val(response.name);

						$('#productName' + row).append("<option value='" + id + "'>" + name + '</option>');
					}
				}
			});
		}
