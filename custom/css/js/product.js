...
*/
			...
			if(quantity == "") {
				...
				if(price == "") {
				$("#original").after('<p class="text-danger">Price field is required</p>');
				$('#original').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#original").find('.text-danger').remove();
				// success out for form
				$("#original").closest('.form-group').addClass('has-success');
			}	// /else
...
*/
			...
			if(categoryName && productName && quantity && rate && productStatus) {
	...
	if(productId) {
		...
		$.ajax({
		  ...
		  // 		allowedFileExtensions: ["jpg", "png", "gif", "JPG", "PNG", "GIF"]
				...
				$("#editOrigal").val(response.originalPrice);
				// brand name
				...
				$("#editProductForm").unbind('submit').bind('submit', function() {
					...
					if(price == "") {
						$("#editOrigal").after('<p class="text-danger"> Original field is required</p>');
						$('#editOrigal').closest('.form-group').addClass('has-error');
					}	else {
						// remov error text field
						$("#editOrigal").find('.text-danger').remove();
						// success out for form
						$("#editOrigal").closest('.form-group').addClass('has-success');
					}	// /else
					...
					if(productName && quantity && rate && price && categoryName && productStatus) {
						// submit loading button
